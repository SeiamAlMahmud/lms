...
exports.analyticsService = {
    getGlobalAnalytics: async () => {
        ...
        const [totalCourses, totalActiveStudents, rawGrowth, rawPopular, rawRevenue, courseList] = await Promise.all([
            prisma_1.prisma.course.count({ where: { isDeleted: false } }),
            prisma_1.prisma.user.count({ where: { role: client_1.UserRole.STUDENT, status: client_1.UserStatus.ACTIVE } }),
            prisma_1.prisma.$runCommandRaw({
        ...
        const growthDocs = (rawGrowth?.cursor?.firstBatch ?? []);
        ...
        const popularDocs = (rawPopular?.cursor?.firstBatch ?? []);
        const revenueDocs = (rawRevenue?.cursor?.firstBatch ?? []);
        ...
        const completionStats = await prisma_1.prisma.enrollment.groupBy({
            by: ["courseId", "status"],
            _count: {
                _all: true,
            },
        ...
        for (const stat of completionStats) {
            ...
            if (stat.status === client_1.EnrollmentStatus.COMPLETED) {
    ...
    getInstructorAnalytics: async (instructorId) => {
        ...
        const stats = await prisma_1.prisma.enrollment.groupBy({
            by: ["courseId", "status"],
            where: {
